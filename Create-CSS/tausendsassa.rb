...
# 'formatindex' hat den Wert 0 (für das 16x12 Bildformat) oder 1 (für das 16x09 Bildformat).

def masse_eintragen(weg, openerflag, formatindex, rahmenarray, textfeldarray, autocsspfad, cssdatei)
  # Die if-Kondition entscheidet, welche Maße verwendet werden.
  # 'weg' kann zur Zeit die Werte 'vierk' und 'gross' annehmen.
    if openerflag == true
        require "./masse_opener_" + weg
    else
        require "./masse_" + weg
    end
  ...
  laengenhash[:displaywidthstr]        = displaywidth
...
# Ende von def masse_eintragen().


# Hauptprogramm:

# formatindex = 0: Bildformat 16x12
# formatindex = 1: Bildformat 16x09
# formatindex ..... weitere Definitionen möglich.

# Das Array mit den Dateinamen aller css-Dateien:
...
# Zwei Durchgänge für die je 32 css-Dateien der beiden Bildschirmgrößen:

  openerflag    = false   
  require_relative "suchenundersetzen"
  stringtable = ["gross","vierk"]

  # Iteration über ["gross","vierk"]. Die errechneten Werte
  # werden in die css-Datei eingetragen und die Datei wird gespeichert: 

  stringtable.each do |word|      
    puts ("Der Durchgang für die Größe '#{word}'")

    # require "./masse_opener_" + word
    # require "./masse_" + word

    hasharray.each{|item|

     masse_eintragen(word,
        item[:openerflag],
        item[:formatindex],
        item[:rahmenarray],
        item[:textfeldarray],
        item[:autocsspfad],
        item[:cssdatei])
  } # Die Klammer beendet den ersten Iterationsschritt.
    # Der zweite Durchlauf beginnt.
  print "Alle Formate mit '#{word}' bearbeitet. \n"
